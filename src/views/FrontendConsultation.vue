...
import { startSessionAPI, getSessionListAPI, deleteSessionAPI, getSessionDetailAPI, getSessionEmotionAPI } from '@/apis/frontend'
import MarkdownRenderer from '@/components/MarkdownRenderer.vue'
import { fetchEventSource } from '@microsoft/fetch-event-source'
const incoRobotUrl = new URL('@/assets/images/robot-fill.png', import.meta.url).href
const iconLikeUrl = new URL('@/assets/images/like.png', import.meta.url).href
const iconUsersUrl = new URL('@/assets/images/users.png', import.meta.url).href
const messages = ref([])
...
const startNewSession = (message) => {
  ...
  startSessionAPI(sessionParams).then(res => {
    const sessionData = {
...
const ctrl = new AbortController() // 控制流式请求的控制器
// 流式对话
const startAiResponse = (sessionId, userMessage) => {
  ...
  fetchEventSource('/api/psychological-chat/stream', {
    ...
    error:(err) => {handleError(err || "AI回复失败")
      throw err
    },
    onclose:()=> {
      // 情绪分析
...
const handleSessionClick = (session) => {
  getSessionDetailAPI(session.id).then(res => {
    messages.value = res
  })
  const sessionData = {
    ...
    <!-- AI助手信息 -->
     <div class="ai-assistant-info">
      <div class="breathing-circle">
        <el-image :src="incoRobotUrl" style="width: 25px; height: 25px;" alt="AI助手"/>
      </div>
      ...
      <div class="emotion-garden">
        ...
        <div class="warm-tips">
          <div class="emotion-status-text">
            ...
            <span class="status-emotion">{{ currentEmotion.isNegative ? '需关注' : '很不错' }}</span>
          </div>
          <div class="emotion-intensity">
            <span class="intensity-dots">
              <span v-for="dot in 3" :key="dot" class="dot" :class="{'active':getIntensityClass(currentEmotion.riskLevel) >= dot}"></span>
            </span>
          ...
          <div class="session-item" v-for="session in sessionList" :key="session.id" @click="handleSessionClick(session)">
            <div class="session-info">
              ...
              <div class="session-actions">
                <el-button type="danger" text size="mini" @click="handleDeleteSession(session.id)">
    ...
    <div class="chat-header">
      <div class="header-left">
        <div class="chat-avatar">
          <el-image :src="iconLikeUrl" style="width: 30px; height: 30px;"/>
               </div>
        <div class="chat-info">
          <h2>宁渡AI助手</h2>
          <p>您的贴心AI心理健康助手</p>
      ...
      <!-- 欢迎用语 -->
       <div class="message-item ai-message" v-if="messages.length === 0">
        <div class="message-avatar">
          <el-image :src="incoRobotUrl" style="width: 18px; height: 18px;"/>
        </div>
       ...
       <div class="message-item" v-for="msg in messages" :key="msg.id" :class="msg.senderType === 1 ? 'user-message' : 'ai-message'">
        <div class="message-avatar">
          <el-image :src="msg.senderType === 1 ? iconUsersUrl : iconLikeUrl" style="width: 18px; height: 18px;"/>
        </div>
        <div class="message-content">
          <div class="message-bubble">
            <!-- AI助手输入中 -->
            <div class="typing-indicator" v-if="msg.senderType === 2 && isAiTyping && !msg.content"></div>
            <!-- 错误消息 -->
    ...
    <!-- 信息输入区域 -->
     <div class="chat-input">
      <div class="input-container">
        <el-input v-model="userMessage" placeholder="请输入您想要分享的内容..." type="textarea" :rows="3" :disabled="isAiTyping" @keyup="handleKeyDown" class="message-input" clearable>
        </el-input>
      ...
      <el-button type="primary" @click="sendMessage" :disabled="!userMessage.trim() || userMessage.length > 500" class="send-btn"><el-icon><Promotion/></el-icon></el-button>
     </div>
  </div>
</div>
</template>
