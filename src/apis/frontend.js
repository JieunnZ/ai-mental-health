import serve from '@/untils/request'
// 注册接口
export const registerAPI = (data) => {
  return serve.post('/user/add', data)
}
// 创建新的会话
export const startSessionAPI = (data) => {
  return serve.post('/psychological-chat/session/start', data)
}
// 分页查询咨询会话
export const getSessionListAPI = (params) => {
  return serve.get('/psychological-chat/sessions', { params })
}
// 删除咨询会话
export const deleteSessionAPI = (sessionId) => {
  return serve.delete(`/psychological-chat/sessions/${sessionId}`)
}
// 获取会话消息列表
export const getSessionDetailAPI = (sessionId) => {
  return serve.get(`/psychological-chat/sessions/${sessionId}/messages`)
}
// 获取会话情绪分析结果
export const getSessionEmotionAPI = (sessionId) => {
  return serve.get(`/psychological-chat/session/${sessionId}/emotion`)
}
//创建或更新情绪日记
export const addEmotionDiaryAPI = (data) => {
  return serve.post('/emotion-diary', data)
}
// 查询知识文章列表
export const getKnowledgeListAPI = (params) => {
  return serve.get('/knowledge/article/page', { params })
}
// 获取知识文章详情
export const getKnowledgeDetailAPI = (articleId) => {
  return serve.get(`/knowledge/article/${articleId}`)
}


